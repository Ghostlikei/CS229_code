...
###### BEGIN YOUR CODE ######
# TODO:
# Initialize value function array with small random values (0 to 0.10)
value_function = np.random.uniform(0, 0.10, size=NUM_STATES)

# Initialize transition probabilities uniformly (1/NUM_STATES)
transition_probs = np.ones((NUM_STATES, NUM_ACTIONS, NUM_STATES)) / NUM_STATES

# Initialize state rewards to zero
state_rewards = np.zeros(NUM_STATES)

# Initialize Transition Counts
transitions_count = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))

# Initialize Rewards Count and Sum
rewards_count = np.zeros(NUM_STATES)
rewards_sum = np.zeros(NUM_STATES)
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    for action in range(NUM_ACTIONS):
        ...
        for next_state in range(NUM_STATES):
            expected_value += transition_probs[state, action, next_state] * value_function[next_state]
        expected_values[action] = expected_value

    chosen_action = np.argmax(expected_values)
    ###### END YOUR CODE ######

    # Get the next state by simulating the dynamics
    state_tuple = cart_pole.simulate(chosen_action, state_tuple)
    ...
    transitions_count[state, chosen_action, new_state] += 1

    # Update Rewards Count and Sum
    rewards_count[new_state] += 1
    rewards_sum[new_state] += R
    ###### END YOUR CODE ######
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        # print(f"Tp before update: count: {transitions_count}\n prob: {transition_probs}")
        for s in range(NUM_STATES):
            for a in range(NUM_ACTIONS):
                sum = np.sum(transitions_count[s, a, :])
                if sum > 0:
                    transition_probs[s, a, :] = transitions_count[s, a, :] / sum

            if rewards_count[s] > 0:
                state_rewards[s] = rewards_sum[s] / rewards_count[s]
        # print(f"Tp after update: count: {transitions_count}\n prob: {transition_probs}")
        # print(f"Reward after update: count: {rewards_count}\n prob: {state_rewards}")
        ###### END YOUR CODE ######
        ...
        ###### BEGIN YOUR CODE ######
        # print(f"Value function before update: {value_function}")
        iter = 0
        while True:
            iter += 1
            delta = 0
            for s in range(NUM_STATES):
                ...
                for a in range(NUM_ACTIONS):
                    expected_value = 0
                    for next_state in range(NUM_STATES):
                        expected_value += transition_probs[s, a, next_state] * value_function[next_state]
                    if expected_value > max_value:
                        max_value = expected_value
                # print(f"Max next state value: {max_value}")
                value_function[s] = state_rewards[s] + GAMMA * max_value
                delta = max(delta, abs(old_value - value_function[s]))

            if delta < TOLERANCE:
                break
            # print(f"Value function after update: {value_function}")
